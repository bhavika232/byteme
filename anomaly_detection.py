...
def get_teammate_data():
    """
    Pulls real instance data from teammate's backend.
    Her format: {instance_id, state, cpu, memory, ...}
    We convert it to our pipeline format.
    """
    try:
...
def convert_aws_record(record):
    ...
    cpu      = record.get("usage") or record.get("cpu") or random.uniform(20, 55)
    raw_cost = record.get("cost", 0)
    if raw_cost == 0:
        raw_cost = (cpu / 100) * 5
    cost_inr = round(raw_cost * USD_TO_INR, 2)
...
def load_from_live_feed(raw_records):
    converted = [convert_aws_record(r) for r in raw_records]
    by_date   = {}
...
def run_analysis(raw_records=None):
    if raw_records:
        print("📥 Using live feed records from teammate...")
    ...
    result = {
        ...
        "summary": {
            ...
            "idle_count":       sum(1 for d in data if d["cpu"] < 35),
            "highest_spike": {
                "date": highest["date"],
                "cost": highest["cost"]
            }
